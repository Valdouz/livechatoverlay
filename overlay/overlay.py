...
class RoundedVideoView(QGraphicsView):
    """QGraphicsView avec coins arrondis via setMask."""
    ...
    def showEvent(self, event):
        ...
        # Applique le masque après affichage
...
class AuthLabel(QLabel):
    """Label auteur avec fond semi-transparent et coins arrondis."""
...
class Overlay(QWidget):
    ...
    def __init__(self):
        ...
        self.auth = AuthLabel()
        self.auth.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint | Qt.Tool)
        self.auth.setAttribute(Qt.WA_TranslucentBackground, True)
        self.auth.setStyleSheet("color:white; font:bold 13px sans-serif; background:transparent; padding:3px 10px;")
        self.auth.hide()

        self.caption = QLabel()
        self.caption.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint | Qt.Tool)
        ...
        # QGraphicsVideoItem : supporte rotation + coins arrondis
        self.vid_scene = QGraphicsScene()
        self.vid_item  = QGraphicsVideoItem()
        self.vid_scene.addItem(self.vid_item)
        self.vid_item.nativeSizeChanged.connect(self._on_native_size)
        self.vid_view  = RoundedVideoView(self.vid_scene)
        self.vid_view.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint | Qt.Tool)
        self.vid_view.setStyleSheet("background: black; border: none;")
        self.vid_view.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.vid_view.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.vid_view.setRenderHint(QPainter.Antialiasing)
        self.vid_view.hide()
        self.player.setVideoOutput(self.vid_item)
        self.surface = None
    ...
    def _display(self, data):
        ...
        if data.get("type") == "video":
            self._play_video(url, author, message)
        else:
            self._fetch_image(url, author, duration, message)

    def _fetch_image(self, url, author, duration, message=""):
        ...
        def done():
            ...
            if bytes(raw[:6])[:3] == b"GIF":
                ...
                if movie.isValid() and orig.width() > 0:
                    ...
                    self._show_caption(message, w, h)
                    movie.start()
    ...
    def _play_video(self, url, author, message=""):
        self._video_w = self.max_w
        self._video_h = int(self.max_w * 9 / 16)
        self._rotation = 0

        self._vid_author = author
        self._vid_message = message
        self._rotation = 0
        self._place_vid(self._video_w, self._video_h)
        self.vid_view.show()
    ...
    def _on_native_size(self, size):
        ...
        nw, nh = size.width(), size.height()

        # WMF (Windows) applique souvent la rotation automatiquement :
        # nativeSize() retourne alors déjà les dimensions dans la bonne orientation.
        # On ne tourne manuellement que si la vidéo est encore en paysage (nw > nh)
        # alors qu'une rotation portrait (90/270) est attendue.
        needs_rotation = self._rotation in (90, 270) and nw > nh

        if needs_rotation:
            # Frames stockées en landscape → rotation manuelle pour affichage portrait
            self.vid_item.setSize(QSizeF(nw, nh))
            t = QTransform()
            t.translate(nh / 2, nw / 2)
            t.rotate(self._rotation)
            t.translate(-nw / 2, -nh / 2)
            self.vid_item.setTransform(t)
            display_w, display_h = nh, nw
        else:
            # WMF a déjà appliqué la rotation (ou pas de rotation nécessaire)
            self.vid_item.setTransform(QTransform())
            self.vid_item.setSize(QSizeF(nw, nh))
            display_w, display_h = nw, nh

        scaled = QSizeF(display_w, display_h).scaled(self.max_w, self.max_h, Qt.KeepAspectRatio)
        w, h = int(scaled.width()), int(scaled.height())
        self._video_w, self._video_h = w, h

        self.vid_view.setGeometry(self.sw - w - MARGIN, self.sh - h - MARGIN, w, h)
        self.vid_view.setSceneRect(0, 0, display_w, display_h)
        self.vid_view.fitInView(0, 0, display_w, display_h, Qt.IgnoreAspectRatio)
        self._show_auth(self._vid_author, w, h)
        self._show_caption(self._vid_message, w, h)

    def _place_vid(self, w, h):
        from PyQt5.QtCore import QSizeF
        self.vid_item.setSize(QSizeF(w, h))
        self.vid_view.setGeometry(self.sw - w - MARGIN, self.sh - h - MARGIN, w, h)
        self.vid_view.setSceneRect(0, 0, w, h)
    ...
    def _show_caption(self, text, content_w, content_h):
        if not text:
            self.caption.hide()
            return
        self.caption.setText(text)
        self.caption.setFixedWidth(content_w)
        cap_h = self.caption.sizeHint().height()
        if cap_h <= 0:
            cap_h = 34
        x = self.sw - content_w - MARGIN
        media_y = self.sh - content_h - MARGIN
        y = media_y - cap_h - 6
        self.caption.setGeometry(x, y, content_w, cap_h)
        self.caption.hide()
        self.caption.show()
    ...
    def _show_auth(self, text, content_w, content_h):
        ...
        # Masquer/remontrer force Windows à le mettre devant vid_view
        self.auth.hide()
        self.auth.show()
